...
def insert_listing(listing):
    ...
    try:
        with get_connection() as conn:
            ...
            conn.commit()
            logger.info(f"Добавлено в listings: {listing['title']} (id={listing['id']})")
            return True
    except sqlite3.IntegrityError:
        logger.debug(f"Уже существует (id={listing['id']}): {listing['title']}")
        return False
    except Exception as e:
        logger.error(f"Ошибка при вставке объявления: {e}")
        return False
...
def add_subscriber(chat_id):
    ...
    try:
        ...
        logger.error(f"Ошибка при добавлении подписчика {chat_id}: {e}")
def get_subscribers():
...
def get_new_listings():
    ...
    try:
        with get_connection() as conn:
            ...
            if listings:
                logger.info(f"Найдено новых объявлений: {len(listings)}")
        ...
        logger.error(f"Ошибка при получении новых объявлений: {e}")
        return []
